...
export default function OldSinlgleCard({customeClass,data}:SINGLE_INTERFACE) {
  ...
  return (
    ...
    >
        ...
        <div className='work_image absolute h-full w-full inset-0 top-0 left-0' >
         ...
         {
          !isResponsive && (
             <div className={`absolute inset-32 flex items-center z-30 justify-center transition-opacity duration-300 ${ (isHovered || isResponsive) ? 'opacity-0' : 'opacity-100'}`}>
              <h2 className="text-white md:text-[3vw] text-[5vw] font-bold">{data.name}</h2>
         </div>
          )
         }
      ...
      >
        ...
        <p className="relative w-full text-center h-[20%] py-4  md:text-[1.1vw] text-[3.6vw] text-black bg-white ">{data.description}</p>
      </div>
